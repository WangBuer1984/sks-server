...
| POST | `/api/profile/confirm` | `{sessionId, turns}` | `null` |
...
- `ActiveProfileView`：`{calibrated, version, calibratedAt, content}`。无 active 行返 `calibrated=false`（非 404）。`content` 已剥掉 `_` 前缀 meta 键（`_interview_turns` 等）。
